...
// 最大流最小コスト(Petr's impl, 未検証)
int minCostMaxFlow(int numVertices, vector<vector<int> >& cap, vector<vector<int> >& cost, int s, int v)
{
  int res = 0;
  while (true) {
    vector<int> prev(numVertices);
    vector<int> dist(numVertices, 1000000000);
    dist[s] = 0;
    while (true) {
      bool updated = false;
      REP(i, numVertices) if(dist[i] != INF) REP(j, numVertices) {
        if (cap[i][j] > 0 && dist[i] + cost[i][j] < dist[j]) {
          dist[j] = dist[j] + cost[i][j];
          prev[j] = i;
          updated = true;
        }
      if (!updated) break;
